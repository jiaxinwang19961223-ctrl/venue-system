...
class MemberCard(Base):
    ...
    card_type = Column(String(30), nullable=False, comment="卡类型: stored(储值卡)/month(月卡)/season(季卡)/year(年卡)")
    total_times = Column(Integer, default=0, comment="总次数（次卡，已弃用）")
    used_times = Column(Integer, default=0, comment="已用次数（已弃用）")
    stored_value = Column(Float, default=0, comment="储值总额（储值卡）")
    used_value = Column(Float, default=0, comment="已消费金额（储值卡）")
